...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function printWithoutSpaces($element) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function addAction($idProject, $idUser, $title, $description, $date) {
        global $db;
        $addActionSQL = "INSERT INTO actionsinproject(idProject, idAuthor, title, description, dateAction) VALUES (:idProject, :idAuthor, :title, :description, :dateAction)";
    ...
    function deleteAction($idAction) {
        global $db;
        $deleteActionSQL = "DELETE FROM actionsinproject WHERE id = :id";
        $deleteAction = $db->prepare($deleteActionSQL);
        $deleteAction->execute(['id' => $idAction]);

        if ($deleteAction) {
            echo 'L\'action a bien été supprimé !</br>';
            echo '<script>window.location="progress.php?year='.$_GET['year'].'"</script>';
        } else {
            echo 'L\'action n\'a pas été supprimé !</br>';
        }
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function getAllStudent() {
        global $db;
        $getAllStudentSQL = "SELECT * FROM users WHERE promo != 0";
        $getAllStudent    = $db->prepare($getAllStudentSQL);
        $getAllStudent->execute();
        $students = array();

        $i = 0;
        while ($e = $getAllStudent->fetch()) {
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function printDOCUMENTS($path, $element, $title) {
        echo '
            <a href="'.$path.$element.'" class="element" target="_blank">
    ...
    function printCODES($path, $element, $title) {
        echo '
            <a href="'.$path.$element.'" class="element" target="_blank">
    ...
    function printElement($path, $element) {
        ...
        if ($fileExtension == "zip")       printZIP($path, $element, $title);
            ...
            if (basename($path) == 'files') printDOCUMENTS($path, $element, $title);
            if (basename($path) == 'codes') printCODES($path, $element, $title);
