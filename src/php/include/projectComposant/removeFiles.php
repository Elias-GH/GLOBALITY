<?php
    ...
    if (isset($_POST['formDeleteCodes'])) {
        include '../../../../src/php/include/projectComposant/blockForm/formDeleteCodes.php';
    }
    ...
    for ($i = 2; $i <= $cptCodes+1; $i++) {
        $pathToDelete = '../../../../src/data/projects/'.$project['name'].'/codes/';
        if (isset($_POST['formDeleteCodes-'.$i])) {
            $_GET['year'] = htmlspecialchars($_GET['year']);
            echo 'formDeleteCodes-'.$i.' has been confirmed </br>';
            $tmpPath = $pathToDelete;
            $tmpPath = $tmpPath.$allCodes[$i];
            echo '$allCodes['.$i.'] : '.$allCodes[$i].'</br>';
            echo 'tempPath : '.$tmpPath.'</br>';
            if(!unlink($tmpPath)) {
                echo $allCodes[$i].' n\'a pas été supprimé </br>';
            } else {
                echo $allCodes[$i].' a été supprimé </br>';
                echo '<script>window.location="shareFiles.php?year='.$_GET['year'].'"</script>';
            }
        }
    }
