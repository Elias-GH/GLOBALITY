...
<script type="text/javascript">
    $(document).ready(function () {
        $('#addNewFile').on('change', function(e) {
            var files = $(this)[0].files;
            if (files.length >= 2) {
                $('#label-addNewFile').text(files.length + " fichiers");
            } else {
                $('#label-addNewFile').text(files.length + " fichier");
            }
        });
    });
</script>
...
<div class="showFiles" id="showFiles" style="display: grid;">
    ...
    <div class="part" id="codes" style="display: none;">
        <h3 class="title">Codes</h3>
        <form action="#" method="post">
            <input type="submit" name="formDeleteCodes" id="formDeleteCodes" value="" style="display: none">
            <label for="formDeleteCodes"><i class="fas fa-trash"></i></label>
        </form>
        <ul class="list">
            <?php
                if ($cptCodes == 0)  {
                    echo '<p class="information">Aucun code disponible </br></p>';
                } else {
                    for($i = 2; $i <= $cptCodes+1; $i++) {
                        printElement($pathToFiles.'codes/', $allCodes[$i]);
                    }
                }
             ?>
        </ul>
